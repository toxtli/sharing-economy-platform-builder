...
class Date {
	...
	public function control_button() {
		...
		<li class="list-group-item" data-type="<?php echo __CLASS__ ?>" for="Date">
			...
			<a title="Date" rel="Date" class="add pull-right add-form-field" data-template='Date' href="#"><i class="fa fa-plus-circle ttipf" title=""></i></a>
		</li>
	    <?php
...
?>
	...
	public function field_render_html($params = array()) {
		...
		<div id="<?php echo $params['id'] ?>" class='form-group <?php if (isset($params['conditioned'])) echo " conditioned hide "?>' data-cond-fields="<?php echo $condition_fields ?>" data-cond-action="<?php echo $cond_action.':'.$cond_boolean ?>" >
			...
			<div class="input-group date" id="dp_<?php echo $datepicker_field_id ?>">
				   <input type="text" value="" class="form-control"  name='submitform[<?php echo $params['id'] ?>]' id='<?php echo $datepicker_field_id ?>' <?php echo required($params) ?> >
		...
		<script type='text/javascript'>
			jQuery(function($) {
				$('#dp_<?php echo $datepicker_field_id ?>').datetimepicker();
			});
		</script>
		<?php } else { ?>
		<script type='text/javascript'>
