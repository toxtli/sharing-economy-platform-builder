...
class Rating {
	...
	public function control_button() {
		ob_start();
		?>
		<li class="list-group-item" data-type="<?php echo __CLASS__ ?>" for="Rating">
			<span class="lfi lfi-name"><i class="fa fa-star-half"></i></span> Rating
			<a title="Rating" rel="Rating" class="add pull-right add-form-field" data-template='Rating' href="#"><i class="fa fa-plus-circle ttipf" title=""></i></a>
	</li>
	    <?php
...
?>
		<li class="list-group-item" data-type="<?php echo __CLASS__ ?>" id="field_<?php echo $fieldindex; ?>">
			...
			<div class="field-preview">
				<script type='text/javascript'>
					jQuery(document).ready(function () { console.log('Hello'); jQuery('#very_rand_rating_div').rateit({ max: 5, step: 2, backingfld: '#very_rand_rating_field' }); });
				</script>
				<?php 	
			...
			<div class="cog" id="cog_<?php echo $fieldindex; ?>" style='display: none'>
				<fieldset>
					...
					<div class='form-group'>
						...
						<div class='row'>
							<div class='col-md-6'>
								<input type='text' class='form-control' value='<?php echo $field_infos[$fieldindex]['max_rating'] ?>' placeholder='Maximum rating' name='contact[fieldsinfo][<?php echo $fieldindex ?>][max_rating]' />
							</div>
							<div class='col-md-6'>
								<input type='text' class='form-control' value='<?php echo $field_infos[$fieldindex]['rating_steps'] ?>' placeholder='Rating steps' name='contact[fieldsinfo][<?php echo $fieldindex ?>][rating_steps]' />
							</div>
						</div>
					</div>
					<div class="form-group">
	...
	public function field_preview_html($params = array()) {
		...
		<div class='form-group'>
			<div id='very_rand_rating_div'></div>
		</div>
		<?php
	...
	public function field_render_html($params = array()) {
		...
		<div id="<?php echo $params['id'] ?>" class='form-group <?php if (isset($params['conditioned'])) echo " conditioned hide "?>' data-cond-fields="<?php echo $condition_fields ?>" data-cond-action="<?php echo $cond_action.':'.$cond_boolean ?>" >
			...
			<input type='hidden' name='submitform[<?php echo $params['id'] ?>]' id='<?php echo $params['id'] ?>_rating_field' <?php echo required($params) ?>/>
			<div class='form-group'>
				<div id='<?php echo $params['id']?>_rating_div'></div>
			</div>
		</div>
		<script type='text/javascript'>
			jQuery(function () { jQuery('#<?php echo $params['id']?>_rating_div').rateit({ max: <?php echo $max_rating ?>, step: <?php echo $rating_steps ?>, backingfld: '#<?php echo $params['id']?>_rating_field' }); });
		</script>
		<?php
		$field_render_html = ob_get_clean();
		return $field_render_html;
			...
			<div class="cog" id="cog_{{ID}}" style="display: none;">
				<fieldset>
					...
					<div class='form-group'>
						...
						<div class='row'>
							<div class='col-md-6'>
								<input type='text' class='form-control' value='' placeholder='Maximum rating' name='contact[fieldsinfo][{{ID}}][max_rating]' />
							</div>
							<div class='col-md-6'>
								<input type='text' class='form-control' value='' placeholder='Rating steps' name='contact[fieldsinfo][{{ID}}][rating_steps]' />
							</div>
						</div>
					</div>
					<div class="form-group">
