...
class FullName {
	...
	public function control_button() {
		ob_start();
		?>
		<li class="list-group-item" data-type="<?php echo __CLASS__ ?>" for="FullName">
			<span class="lfi lfi-name"><i class="fa fa-user-circle-o"></i></span> Full Name
			<a title="FullName" rel="FullName" class="add pull-right add-form-field" data-template='FullName' href="#"><i class="fa fa-plus-circle ttipf" title=""></i></a>
	</li>
	    <?php
		...
		<div id="<?php echo $params['id'] ?>" class='form-group <?php if (isset($params['conditioned'])) echo " conditioned hide "?>' data-cond-fields="<?php echo $condition_fields ?>" data-cond-action="<?php echo $cond_action.':'.$cond_boolean ?>" >
			...
			<div class='form-group' >
				<div class='row'>
					<div class='col-md-2'>
						<label for='title'>Title: </label>
						<input class='form-control' type='text' name='submitform[<?php echo $params['id'] ?>][title]' id='title' <?php echo required($params) ?> />
					</div>
					<div class="col-md-5">
						<label for='first-name'>First name: </label>
						<input class='form-control' type='text' name='submitform[<?php echo $params['id'] ?>][first_name]' id='first_name' <?php echo required($params) ?> />
					</div>
					<div class="col-md-5">
						<label for='last-name'>Last name: </label>
						<input class='form-control' type='text' name='submitform[<?php echo $params['id'] ?>][last_name]' id='last_name' <?php echo required($params) ?> />
					</div>
				</div>
			</div>
		</div>
