...
class Daterange {
	...
	public function control_button() {
		ob_start();
		?>
		<li class="list-group-item" data-type="<?php echo __CLASS__ ?>" for="Daterange">
			<span class="lfi lfi-name"><i class="fa fa-calendar-plus-o"></i></span> Date Range
			<a title="Daterange" rel="Daterange" class="add pull-right add-form-field" data-template='Daterange' href="#"><i class="fa fa-plus-circle ttipf" title=""></i></a>
		</li>
	    <?php
...
?>
	...
	public function field_render_html($params = array()) {
		...
		$datepicker_field1_id = "datepicker1_{$params['id']}";
		$datepicker_field2_id = "datepicker2_{$params['id']}";
		$condition_fields = '';
		$cond_action = '';
		$cond_boolean = '';
		...
		<div id="<?php echo $params['id'] ?>" class='form-group <?php if (isset($params['conditioned'])) echo " conditioned hide "?>' data-cond-fields="<?php echo $condition_fields ?>" data-cond-action="<?php echo $cond_action.':'.$cond_boolean ?>" >
			...
			<div class='row' >
				<div class="col-md-6">
					<input class='form-control datepicker' type='text' name='submitform[<?php echo $params['id'] ?>]' id='<?php echo $datepicker_field1_id ?>' <?php echo required($params) ?>/>
				</div>
				<div class="col-md-6">
					<input class='form-control datepicker' type='text' name='submitform[<?php echo $params['id'] ?>]' id='<?php echo $datepicker_field2_id ?>' <?php echo required($params) ?>/>
				</div>
			</div>
		</div>
		...
		<script type='text/javascript'>
			jQuery(document).ready(function($){
				...
				$('#<?php echo $datepicker_field1_id ?>').datetimepicker();
				$('#<?php echo $datepicker_field2_id ?>').datetimepicker({
					useCurrent: false //Important! See issue #1075
				});
				$("#<?php echo $datepicker_field1_id ?>").on("dp.change", function (e) {
					$('#<?php echo $datepicker_field2_id ?>').data("DateTimePicker").minDate(e.date);
				});
				$("#<?php echo $datepicker_field2_id ?>").on("dp.change", function (e) {
					$('#<?php echo $datepicker_field1_id ?>').data("DateTimePicker").maxDate(e.date);
				});
