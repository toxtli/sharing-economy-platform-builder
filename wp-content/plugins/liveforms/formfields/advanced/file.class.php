...
class File {
	...
	public function control_button() {
		ob_start();
		?>
		<li class="list-group-item" data-type="<?php echo __CLASS__ ?>" for="File">
			<span class="lfi lfi-name"><i class="fa fa-upload"></i></span> File
			<a title="File" rel="File" class="add pull-right add-form-field" data-template='File' href="#"><i class="fa fa-plus-circle ttipf" title=""></i></a>
		</li>
		<?php
				...
				<fieldset>
					...
					<div class="form-group">
						...
						<div
							...
							<div class="form-group">
								<label><input type="text" name="contact[fieldsinfo][<?php echo $fieldindex ?>][filesize]" placeholder="File size limit" value="<?php echo (isset($field_infos[$fieldindex]['filesize']) ? $field_infos[$fieldindex]['filesize'] : '') ?>"/> Filesize</label>
								<label><input type="text" name="contact[fieldsinfo][<?php echo $fieldindex ?>][extensions]" placeholder="Allowed extensions" value="<?php echo (isset($field_infos[$fieldindex]['extensions']) ? $field_infos[$fieldindex]['extensions'] : '') ?>"/> Extensions</label>
							</div>
						</div>
					</div>
				</fieldset>
	...
	public function field_preview_html($params = array()) {
		...
		<div class="input-group">
			...
			<input class="form-control" id="uppp_<?php echo $params['id'] ?>" style="background: #ffffff" type="text" readonly="readonly">
		</div>
		<?php
	...
	public function field_render_html($params = array()) {
		...
		<div id="<?php echo $params['id'] ?>" class='form-group <?php if (isset($params['conditioned'])) echo " conditioned hide "?>' data-cond-fields="<?php echo $condition_fields ?>" data-cond-action="<?php echo $cond_action.':'.$cond_boolean ?>" >
			...
			<div class="fileUpload input-group">
				<span class="input-group-btn">
					<button type="button" class="btn btn-default"><i class="fa fa-folder-open-o"></i> Browse</button>
					<input style="height: 34px;" onchange="jQuery('#uppp_<?php echo $params['id'] ?>').val(jQuery(this).val().replace(/^.*[\\\/]/, ''));" type="file" class="upload"  name='upload[<?php echo $params['id'] ?>]' <?php echo required($params) ?> />
				</span>
