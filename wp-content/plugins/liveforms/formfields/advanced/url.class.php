...
class Url {
	...
	public function control_button() {
		ob_start();
		?>
		<li class="list-group-item" data-type="<?php echo __CLASS__ ?>" for="Url">
			<span class="lfi lfi-name"><i class="fa fa-link"></i></span> URL
			<a title="Url" rel="Url" class="add pull-right add-form-field" data-template='Url' href="#"><i class="fa fa-plus-circle ttipf" title=""></i></a>
		</li>
	    <?php
...
?>
	...
	public function field_render_html($params = array()) {
		...
		<div id="<?php echo $params['id'] ?>" class='form-group <?php if (isset($params['conditioned'])) echo " conditioned hide "?>' data-cond-fields="<?php echo $condition_fields ?>" data-cond-action="<?php echo $cond_action.':'.$cond_boolean ?>" >
			...
			<div class='form-group'>
				<input type='text' class='form-control' name=submitform[<?php echo $params['id'] ?>] id='urlfield_<?php echo $params['id'] ?>' <?php echo required($params) ?> placeholder="http://www.website.com" value='<?php echo isset($params['url']) ? $params['url'] :''  ?>'/>
			</div>
		</div>
		<?php
		$field_render_html = ob_get_clean();
