...
class Name {
	...
	public function control_button() {
		ob_start();
		?>
		<li class="list-group-item" data-type="<?php echo __CLASS__ ?>" for="Name">
			<span class="lfi lfi-name"><i class="fa fa-user"></i></span> Name
			<a title="Name" rel="Name" class="add pull-right add-form-field" data-template='Name' href="#"><i class="fa fa-plus-circle ttipf" title=""></i></a>
		</li>
	    <?php
...
?>
	...
	public function field_render_html($params = array()) {
		...
		<div id="<?php echo $params['id'] ?>" class='form-group <?php if (isset($params['conditioned'])) echo " conditioned hide "?>' data-cond-fields="<?php echo $condition_fields ?>" data-cond-action="<?php echo $cond_action.':'.$cond_boolean ?>" >
			...
			<input type='text'  name='submitform[<?php echo isset($params['id'])?$params['id']:'' ?>]'  value='' <?php echo required($params)?> class='form-control ' />
		</div>
		<?php
		$field_render_html = ob_get_clean();
		return $field_render_html;
