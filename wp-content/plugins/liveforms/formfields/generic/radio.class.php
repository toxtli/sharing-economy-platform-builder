...
class Radio {
	...
	public function control_button() {
		ob_start();
		?>
		<li class="list-group-item" data-type="<?php echo __CLASS__ ?>" for="Radio">
			<span class="lfi lfi-name"><i class="fa fa-dot-circle-o"></i></span> Radio
			<a title="Radio" rel="Radio" class="add pull-right add-form-field" data-template='Radio' href="#"><i class="fa fa-plus-circle ttipf" title=""></i></a>
		</li>
	    <?php
...
?>
	...
	public function field_preview_html($params = array()) {
		...
		?>
            <label style='display: block;clear: both'><input type='radio' disabled="disabled" value='' name='submitform[]' /> <?php echo $label ?></label>
		<?php
        }
		?>
        </div>
	...
	public function field_render_html($params = array()) {
		...
		<div id="<?php echo $params['id'] ?>" class='form-group <?php if (isset($params['conditioned'])) echo " conditioned hide "?>' data-cond-fields="<?php echo $condition_fields ?>" data-cond-action="<?php echo $cond_action.':'.$cond_boolean ?>" >
			...
			foreach ($option_names as $id => $label) {
			?>
				<label  style='display: block;clear: both'><input type='radio' value='<?php echo $option_values[$id] ?>' name='submitform[<?php echo $params['id'] ?>]' <?php echo required($params) ?> /> <?php echo $label ?></label>
			<?php
			}
			?>
			</div>
