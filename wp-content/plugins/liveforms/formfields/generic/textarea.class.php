...
class Textarea {
	...
	public function control_button() {
		ob_start();
		?>
		<li class="list-group-item" data-type="<?php echo __CLASS__ ?>" for="Textarea">
			<span class="lfi lfi-name"><i class="fa fa-align-justify"></i></span> Textarea
			<a title="Textarea" rel="Textarea" class="add pull-right add-form-field" data-template='Textarea' href="#"><i class="fa fa-plus-circle ttipf" title=""></i></a>
		</li>
	    <?php
					...
					<div class="form-group">
						...
						<div
							...
							<input type="text"
								   ...
								   placeholder="Field Required Textarea"
	...
	public function field_render_html($params = array()) {
		...
		<div id="<?php echo $params['id'] ?>" class='form-group <?php if (isset($params['conditioned'])) echo " conditioned hide "?>' data-cond-fields="<?php echo $condition_fields ?>" data-cond-action="<?php echo $cond_action.':'.$cond_boolean ?>" >
			...
			<textarea  name='submitform[<?php echo isset($params['id'])?$params['id']:'' ?>]' class='form-control'  <?php echo required($params)?>  ></textarea>
		</div>
		<?php
		$field_render_html = ob_get_clean();
		return $field_render_html;
					...
					<div class="form-group">
						...
						<div class="req-params" style="display: none">
							<input type="text" name="contact[fieldsinfo][{{ID}}][reqmsg]"
								   placeholder="Field Required Textarea" value="" class="form-control"/>
							<label>Validation:</label>
