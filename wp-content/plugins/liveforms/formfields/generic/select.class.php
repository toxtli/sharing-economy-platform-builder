...
class Select {
	...
	public function control_button() {
		ob_start();
		?>
		<li class="list-group-item" data-type="<?php echo __CLASS__ ?>" for="Select">
			<span class="lfi lfi-name"><i class="fa fa-caret-down"></i></span> Select
			<a title="Select" rel="Select" class="add pull-right add-form-field" data-template='Select' href="#"><i class="fa fa-plus-circle ttipf" title=""></i></a>
		</li>
	    <?php
...
?>
				...
				<fieldset>
					...
					<div class="form-group">
						...
						<div
							...
							<input type="text" name="contact[fieldsinfo][<?php echo $fieldindex ?>][reqmsg]" placeholder="Field Required Message" value="<?php echo $field_infos[$fieldindex]['reqmsg'] ?>"
								   class="form-control"/>
	...
	public function field_preview_html($params = array()) {
		...
		ob_start();
		?>
		<select class="form-control" disabled="disabled" name='submitform[]' <?php echo isset($params['multi'])?'multiple':'' ?> <?php echo required($params) ?>>
	...
	public function field_render_html($params = array()) {
		...
		<div id="<?php echo $params['id'] ?>" class='form-group <?php if (isset($params['conditioned'])) echo " conditioned hide "?>' data-cond-fields="<?php echo $condition_fields ?>" data-cond-action="<?php echo $cond_action.':'.$cond_boolean ?>" >
			...
			<select name='submitform[<?php echo $params['id'] ?>]' <?php echo isset($params['multi'])?'multiple':'' ?> <?php echo required($params) ?>  class='form-control'>
				<option selected='selected' value=''>Please Select</option>
			<?php
			foreach ($option_names as $id => $label) { ?>
				<option value='<?php echo $option_values[$id] ?>'><?php echo $label ?></option>
