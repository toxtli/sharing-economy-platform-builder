...
class Text {
	...
	public function control_button() {
		ob_start();
		?>
		<li class="list-group-item" data-type="<?php echo __CLASS__ ?>" for="Text">
			<span class="lfi lfi-name"><i class="fa fa-minus"></i></span> Text
			<a title="Text" rel="Text" class="add pull-right add-form-field" data-template='Text' href="#"><i class="fa fa-plus-circle ttipf" title=""></i></a>
	</li>
	    <?php
...
?>
				...
				<fieldset>
					...
					<div class="row">
						<div class="form-group col-md-6">
							<select  data-id="<?php echo $fieldindex ?>" name="contact[fieldsinfo][<?php echo $fieldindex ?>][icon-pos][op]" class="form-control icontext" id="icontext" data-selection='<?php echo isset($field_infos[$fieldindex]['icon-pos']['op']) ? $field_infos[$fieldindex]['icon-pos']['op'] : ''  ?>'>
								<option value="no icon">No icon</option>
								<option value="before">Before</option>
								<option value="after">After</option>
							</select>
						</div>
						<input type="hidden" id="icv_<?php echo $fieldindex; ?>" name="contact[fieldsinfo][<?php echo $fieldindex ?>][icon-code]"  value='<?php echo isset($field_infos[$fieldindex]['icon-code']) ? $field_infos[$fieldindex]['icon-code'] : ''  ?>'  data-selection='<?php echo isset($field_infos[$fieldindex]['icon-code']) ? $field_infos[$fieldindex]['icon-code'] : ''  ?>' />
						<div class="form-group col-md-6 hide icon-code-div">
							<div class="btn-group">
								<button type="button" class="btn  btn-default iconpicker-component icon-code" data-target="<?php echo $fieldindex; ?>" ><i class="fa fa-fw fa-heart"></i></button>
								<button type="button" class="icp icp-dd btn btn-default dropdown-toggle" data-selected="<?php echo isset($field_infos[$fieldindex]['icon-code']) ? $field_infos[$fieldindex]['icon-code'] : 'fa-check'  ?>" data-toggle="dropdown">
	...
	public function field_preview_html($params = array()) {
		...
		<?php if($params['icon-pos']['op'] == 'before'){?>
			...
			<span class="input-group-addon" id="icp_<?php echo isset($params['id'])?$params['id']:'{{ID}}'; ?>"><i class='<?php echo isset($params['icon-code']) ? $params['icon-code'] : ''; ?>'></i></span>
	...
	public function field_render_html($params = array()) {
		...
		?>
		    ...
		    <div id="<?php echo $params['id'] ?>" class='form-group <?php if (isset($params['conditioned'])) echo " conditioned hide "?> input-group' data-cond-fields="<?php echo $condition_fields ?>" data-cond-action="<?php echo $cond_action.':'.$cond_boolean ?>">
				...
				<?php if($params['icon-pos']['op'] == 'before'):?>
				<span class="input-group-addon" id=""><i class='<?php echo isset($params['icon-code']) ? "fa ".$params['icon-code'] : ''; ?>'></i></span>
			    <?php endif; ?>
		...
		<li class="list-group-item" data-type="<?php echo __CLASS__ ?>" id="field_{{ID}}"><input type="hidden" name="contact[fields][{{ID}}]" value="{{value}}">
			...
			<div class="cog" id="cog_{{ID}}" style="display: none;">
				<fieldset>
					...
					<div class="row">
					    <div class="form-group col-md-6">
						   <select name="contact[fieldsinfo][{{ID}}][icon-pos][op]" data-id="{{ID}}" class="form-control icontext" id="icontext">
	                           <option value="no icon">No icon</option>
	                           <option value="before">Before</option>
	                           <option value="after">After</option>
						   </select>
						...
						<div class="form-group col-md-6 hide icon-code-div">
						   <input type="hidden" id="icv_{{ID}}" name="contact[fieldsinfo][{{ID}}][icon-code]" />

						    <div class="btn-group">
								<button type="button" class="btn btn-default iconpicker-component icon-code" data-target="{{ID}}" ><i class="fa fa-fw fa-heart"></i></button>
								<button type="button" class="icp icp-dd btn btn-default dropdown-toggle" data-selected="fa-check" data-toggle="dropdown">
									<span class="caret"></span>
									<span class="sr-only">Toggle Dropdown</span>
								</button>
								<div class="dropdown-menu"></div>
