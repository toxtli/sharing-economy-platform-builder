...
class Password {
	...
	public function control_button() {
		ob_start();
		?>
		<li class="list-group-item" data-type="<?php echo __CLASS__ ?>" for="Password">
			<span class="lfi lfi-name"><i class="fa fa-ellipsis-h"></i></span> Password
			<a title="Password" rel="Password" class="add pull-right add-form-field" data-template='Password' href="#"><i class="fa fa-plus-circle ttipf" title=""></i></a>
	</li>
	    <?php
...
?>
	...
	public function field_render_html($params = array()) {
		...
		<div id="<?php echo $params['id'] ?>" class='form-group <?php if (isset($params['conditioned'])) echo " conditioned hide "?>' data-cond-fields="<?php echo $condition_fields ?>" data-cond-action="<?php echo $cond_action.':'.$cond_boolean ?>" >
			<label>Password:</label>
			<input type='password' id="password_<?php echo $params['id'] ?>" name='submitform[<?php echo isset($params['id'])?$params['id']:'' ?>]' class='form-control' value='' <?php echo required($params)?> />
