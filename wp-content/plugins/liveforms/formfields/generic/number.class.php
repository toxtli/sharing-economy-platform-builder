...
class Number {
	...
	public function control_button() {
		ob_start();
		?>
		<li class="list-group-item" data-type="<?php echo __CLASS__ ?>" for="Number">
			<span class="lfi lfi-name"><i class="fa fa-hashtag"></i></span> Number
			<a title="Number" rel="Number" class="add pull-right add-form-field" data-template='Number' href="#"><i class="fa fa-plus-circle ttipf" title=""></i></a>
		</li>
	    <?php
...
?>
	...
	public function field_preview_html($params = array()) {
		ob_start();
		?>
		<input type='number' disabled="disabled" name='submitform[]' class='form-control' value='' />
		<?php
	...
	public function field_render_html($params = array()) {
		...
		<div id="<?php echo $params['id'] ?>" class='form-group <?php if (isset($params['conditioned'])) echo " conditioned hide "?>' data-cond-fields="<?php echo $condition_fields ?>" data-cond-action="<?php echo $cond_action.':'.$cond_boolean ?>" >
			...
			<input type='number' name='submitform[<?php echo isset($params['id'])?$params['id']:'' ?>]' class='form-control' value='' <?php echo required($params)?> />
		</div>
		<?php
		$field_render_html = ob_get_clean();
		return $field_render_html;
