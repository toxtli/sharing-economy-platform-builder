...
class CFGS_Google_CurlIO implements CFGS_Google_IO {
   ...
   * @return CFGS_Google_HttpRequest The resulting HTTP response including the
   * responseHttpCode, responseHeaders and responseBody.
   */
  public function authenticatedRequest(CFGS_Google_HttpRequest $request) {
    $request = CFGS_Google_Client::$auth->sign($request);
   ...
   *
   * @param CFGS_Google_HttpRequest $request the http request to be executed
   * @return CFGS_Google_HttpRequest http request with the response http code, response
   * headers and response body filled in
   * @throws CFGS_Google_IOException on curl or IO error
   */
  public function makeRequest(CFGS_Google_HttpRequest $request) {
    // First, check to see if we have a valid cached version.
    $cached = $this->getCachedRequest($request);
    if ($cached !== false) {
      if (CFGS_Google_CacheParser::mustRevalidate($cached)) {
        $addHeaders = array();
    ...
    if ($curlErrorNum != CURLE_OK) {
      throw new CFGS_Google_IOException("HTTP Error: ($respHttpCode) $curlError");
    }
   ...
   */
  public function setCachedRequest(CFGS_Google_HttpRequest $request) {
    // Determine if the request is cacheable.
    if (CFGS_Google_CacheParser::isResponseCacheable($request)) {
      CFGS_Google_Client::$cache->set($request->getCacheKey(), $request);
      return true;
    }
   ...
   * @visible for testing.
   * @param CFGS_Google_HttpRequest $request
   * @return CFGS_Google_HttpRequest|bool Returns the cached object or
   * false if the operation was unsuccessful.
   */
  public function getCachedRequest(CFGS_Google_HttpRequest $request) {
    if (false == CFGS_Google_CacheParser::isRequestCacheable($request)) {
      false;
    }

    return CFGS_Google_Client::$cache->get($request->getCacheKey());
  }
   ...
   * @param CFGS_Google_HttpRequest $request
   * @return CFGS_Google_HttpRequest Processed request with the enclosed entity.
   */
  public function processEntityRequest(CFGS_Google_HttpRequest $request) {
