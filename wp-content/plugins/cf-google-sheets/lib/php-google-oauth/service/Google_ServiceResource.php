 ...
 *
 */
class CFGS_Google_ServiceResource {
  ...
  /** @var CFGS_Google_Service $service */
  private $service;
   ...
   * @param $name
   * @param $arguments
   * @return CFGS_Google_HttpRequest|array
   * @throws CFGS_Google_Exceptions
   */
  public function __call($name, $arguments) {
    if (! isset($this->methods[$name])) {
      throw new CFGS_Google_Exceptions("Unknown function: {$this->serviceName}->{$this->resourceName}->{$name}()");
    }
    ...
    foreach ($parameters as $key => $val) {
      if ($key != 'postBody' && ! isset($method['parameters'][$key])) {
        throw new CFGS_Google_Exceptions("($name) unknown parameter: '$key'");
      }
    }
    if (isset($method['parameters'])) {
      foreach ($method['parameters'] as $paramName => $paramSpec) {
        if (isset($paramSpec['required']) && $paramSpec['required'] && ! isset($parameters[$paramName])) {
          throw new CFGS_Google_Exceptions("($name) missing required param: '$paramName'");
        }
    ...
    if (isset($method['mediaUpload'])) {
      $media = CFGS_Google_MediaFileUpload::process($postBody, $parameters);
      if ($media) {
    ...
    $url = CFGS_Google_REST::createRequestUri($servicePath, $method['path'], $parameters);
    $httpRequest = new CFGS_Google_HttpRequest($url, $method['httpMethod'], null, $postBody);
    if ($postBody) {
      ...
      if (isset($contentType) && $contentType) {
        ...
        $contentTypeHeader['content-length'] = CFGS_Google_Utils::getStrLen($postBody);
      }
      $httpRequest->setRequestHeaders($contentTypeHeader);
    }

    $httpRequest = CFGS_Google_Client::$auth->sign($httpRequest);
    if (CFGS_Google_Client::$useBatch) {
      return $httpRequest;
    }
    ...
    return CFGS_Google_REST::execute($httpRequest);
  }
