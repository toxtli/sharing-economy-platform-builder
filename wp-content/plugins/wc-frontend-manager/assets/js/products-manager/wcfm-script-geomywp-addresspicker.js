...
jQuery(document).ready(function($) {
  ...
  var wcfm_address = jQuery( "#_wppl_address" ).val();
  $('#wcfm_products_manage_form_geomywp_head').click(function() {
    setTimeout(function() {
			addresspicker = jQuery( "#addresspicker" ).addresspicker();
			addresspickerMap = jQuery( "#wppl-addresspicker" ).addresspicker({
					//regionBias: "fr",
				elements: {
						map:      						"#map",
						lat:      						"#_wppl_lat",
			...
			if( wcfm_address ) {
				...
				geocoder.geocode( { 'address': wcfm_address }, function(results, status) {
					if (status == google.maps.GeocoderStatus.OK) {
						var latlng = results[0].geometry.location;
						jQuery( "#_wppl_lat" ).val(latlng);
						latlng = jQuery( "#_wppl_lat" ).val();
						latlng = ('' != latlng) ? latlng.substring(1, latlng.length-1).split(', ') : ['', ''];
						jQuery( "#_wppl_lat" ).val(latlng[0]);
						jQuery( "#_wppl_long" ).val(latlng[1]);
						returnAddress( latlng[0], latlng[1], 'no' );
